...
`scripts/seed.py` 現在支援兩種執行模式，讓你可以根據不同需求選擇最適合的方式：
...
```bash
# 只測試名言來源，使用非並行模式
python3 scripts/seed.py --quotable 100 --facts 100 --zenquotes 50 \
...
# 需求：某個資料來源有問題，需要除錯
# 建議：只抓取該來源，使用非並行模式
python3 scripts/seed.py --quotable 100 --arxiv 0 --wikipedia 0 \
...
# 需求：在資源有限的機器上執行
# 建議：使用非並行模式，減少資源消耗
python3 scripts/seed.py --total 5000 --no-parallel
```
